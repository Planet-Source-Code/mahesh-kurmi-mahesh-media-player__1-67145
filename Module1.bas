...
' Functions to Drag the Form Freely...
Call ReleaseCapture
Call SendMessage(frm.hwnd, &HA1, 2, 0)
End Sub


Public Sub getSaveFile()
SaveAudiofile = "Audio 1.wav"
Dim i As Integer
Dim j As Integer
Dim exist As Boolean

musicsystem.File1.path = "c:\"
'musicsystem.File1
musicsystem.File1.path = App.path + "\mmp Workarea"
For i = 0 To (musicsystem.File1.ListCount) Step 1
exist = False

SaveAudiofile = "Audio" + str(i + 1) + ".wav"
For j = 0 To (musicsystem.File1.ListCount - 1) Step 1
If (UCase(musicsystem.File1.List(j)) = UCase(SaveAudiofile)) Then
 exist = True
 Exit For
End If
Next
...
4:
SaveAudiofile = "Audio" + str(i + 1) + ".wav"
musicsystem.reclabel.Caption = "AUDIO" + str(i + 1)
musicsystem.rec_count.Left = 790
SaveAudiofile = musicsystem.File1.path + "\" + SaveAudiofile '+ ".wav"
End Sub
